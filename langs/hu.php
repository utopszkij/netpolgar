<?php
// common
DEFINE('LNGDEF',1);
DEFINE('OK','Rendben');
DEFINE('YES','Igen');
DEFINE('NO','Nem');
DEFINE('CANCEL','Mégsem');
DEFINE('CLOSE','Bezár');
DEFINE('TITLE','Cím');
DEFINE('NEXT','következő');
DEFINE('PRIOR','elöző');
DEFINE('FIRST','első');
DEFINE('LAST','utolsó');
DEFINE('PAGES','oldalak');
DEFINE('TOTAL','össesen');
DEFINE('NEXTSTEP','Tovább');
DEFINE('PLEAS_SELECT','Válaszd ki!');
DEFINE('ACCESS_VIOLATION','Hozzáférés letiltott');
DEFINE('DESC','Leírás');
DEFINE('STATE','státusz');
DEFINE('ADMIN','System adminisztrátor');
DEFINE('APPTITLE','Projektek, csoportok, piactér, dokumentumok, események, viták, szavazások, üzenetek');
DEFINE('APPINFO','Szabad Emberek Szabad Társulása');
DEFINE('LIKE','Tetszik, támogatom');
DEFINE('INVALID_NUMBER','Nem megfelelő szám adat');
// man menu
DEFINE('MAINMENU','Főmenű');
DEFINE('HOME','Kezdőlap');
DEFINE('SETUP','Beállítások');
DEFINE('USERS','Felhasználók');
DEFINE('PROJECTS','Projektek');
DEFINE('MARKET','Piactér');
DEFINE('DOCS','Dokumentumok');
DEFINE('DISPUTES','Eszmecserék');
DEFINE('EVENTS','Események');
DEFINE('PROFILE','Profil');
DEFINE('LOGIN','Bejelentkezés');
DEFINE('LOGOUT','Kijelentkezés');
DEFINE('REGIST','Regisztráció');
DEFINE('SEARCH','Keresés');
DEFINE('SEARCH_TXT','keresendő');
DEFINE('GROUPS','Csoportok');
DEFINE('CATEGORIES','Témakörök');
DEFINE('USERGROUPS','Felhasználói csoportok');
DEFINE('ACTIVE','Aktív');
DEFINE('CLOSED','Lezárt');
DEFINE('DRAFT','Tervezet');
DEFINE('OFFER','Kinál');
DEFINE('DEMAND','Keres');
DEFINE('TRANSACTIONS','Tranzakciók');
DEFINE('CHATS','Viták');
DEFINE('MESSAGES','Privát üzenetek');
DEFINE('POLLS','Szavazások');
DEFINE('DECISIONS','Döntések');
DEFINE('FOLLOWS','Elkövetkezők');
DEFINE('OLDS','Korábbiak');
DEFINE('ALL','Összes');
DEFINE('MYDATA','Adataim');
DEFINE('CHANGE_PASSWORD','Jelszó változtatás');
DEFINE('REMOVE_ACCOUNT','Fiók törlése');

// like
DEFINE('LIKE_DETAILS','Támogatók, ellenzők');
DEFINE('LIKE_MEMBER_0','Nem tag');
DEFINE('LIKE_MEMBER_1','Tag');
DEFINE('LIKE_TYPE_dislike','Ellenzi');
DEFINE('LIKE_TYPE_like','Támogatja');

// footer
DEFINE('IMPRESSUM','Impresszum');
DEFINE('DATAPROCESS','Adat kezelési leírás');
DEFINE('LICENCE','Licensz');
DEFINE('SOURCE','Forrás program');
DEFINE('BUGMSG','Programhiba jelzése');
DEFINE('SWRESOURCE','Felhasznált szoftver elemek');

?>
