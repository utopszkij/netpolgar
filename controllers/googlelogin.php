...
class GoogleloginController extends Controller {
	...
	/**
	 * eljárás miután google -al sikeresen bejelentkezett
	 ...
	 * @return UserRecord
	 */
	protected function readOrCreateUser(string $sub, string $name, string $picture): User {
		$user = new User();
		$w = explode(' ',$name);
		$table = new table('users');
		$table->where(['picture','=',$picture]);
		$rec = $table->first();
		if ($rec) {
	...
	public function code(Request &$request) {
		...
		if (isset($token->access_token)) {
	    	...
	    	if (!isset($guser->error)) {
	    	    ...
	    	    // $guser->name, ->id ->picture->data->url
