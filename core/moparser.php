<?php
/**
* .mo nyelvi fájlok beolvasása, értelmezése
*/ 
class moparser {
    ...
    private function _readMOData($bytes)
    {
	 ...
	 /**
	 * mo file parser
	 * @param string $filename teljes file elérési utvonal
	 * @param string $locale  például: 'hu_HU'
	 $ @return ['locale'=> [token =>value, .....]]
	 */  
    public function loadTranslationData(string $filename, string $locale)
    {
        $this->_data      = array();
...
/* test
$moParser = new MoParser();
$data = $moParser->loadTranslationData('admin-hu_HU.mo', 'hu_HU');
